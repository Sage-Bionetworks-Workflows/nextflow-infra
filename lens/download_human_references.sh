...
singularity pull docker://staphb/samtools:1.13
singularity exec -B $PWD samtools*.s* samtools faidx Homo_sapiens_assembly38.fasta
...
singularity exec -B $PWD samtools*.s* samtools faidx -o Homo_sapiens.assembly38.no_ebv.fa Homo_sapiens_assembly38.fasta "${keep_ids[@]}"
...
zgrep 'transcript_type "protein_coding"' ../annot/gencode.v37.annotation.gtf.gz | awk '($3=="exon") {printf("%s\t%s\t%s\n",$1,int($4)-1,$5);}' | sort -T . -t $'\t' -k1,1 -k2,2n | bedtools merge > hg38_exome.bed
cd ..

# snpEff reference
mkdir -p snpeff; cd snpeff
singularity pull docker://resolwebio/snpeff:latest
mkdir -p GRCh38.GENCODEv37
cd GRCh38.GENCODEv37
...
ln ../../annot/gencode.v37.annotation.gtf.gz genes.gtf.gz
ln ../../fasta/Homo_sapiens.assembly38.no_ebv.fa sequences.fa
cd ..
singularity exec -B $PWD snpeff*.s* /opt/snpeff/snpeff/bin/snpEff build -gtf22 -v GRCh38.GENCODEv37 -dataDir ${PWD} -c GRCh38.GENCODEv37/snpEff.config
rm GRCh38.GENCODEv37/sequences.fa
rm GRCh38.GENCODEv37/genes.gtf.gz
rm snpeff*.s*
cd ..
...
python -m pip install --user pyfaidx
python -m pip install --user bcbio-gff
...
python generate_reference_peptidome.py ../annot/gencode.v37.annotation.gff3 ../fasta/Homo_sapiens.assembly38.no_ebv.fa .
